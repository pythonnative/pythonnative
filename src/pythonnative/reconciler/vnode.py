...
_STRUCTURAL = _journal.STRUCTURAL_FIELDS
...
def normalize_children(children: Any, owner: str = "", *, dynamic: bool = False) -> List[Element]:
    ...
    Args:
        children: The value to normalize.
        owner: Label of the element or component the children belong
            to, for diagnostics.
        dynamic: Whether ``children`` itself is a list a component
            returned (as opposed to a container's positional children).
            In dev mode, a dynamic list of two or more elements without
            keys is reported once per ``owner``, as is any nested list
            found among positional children.
    """
    out: List[Element] = []
    is_dev = diagnostics.is_dev()

    def add(item: Any, nested: bool) -> None:
        ...
        for sub in iterator:
            add(sub, True)
        if nested and is_dev:
            _check_unkeyed(out[start:], owner)

    add(children, dynamic)

    if is_dev and len(out) > 1:
        seen: Set[Any] = set()
        for el in out:
            if el.key is None:
                continue
...
def _check_unkeyed(siblings: List[Element], owner: str) -> None:
    """Warn once per owner about a dynamic sibling list whose elements lack keys."""
    if len(siblings) < 2 or all(el.key is not None for el in siblings):
        return
    diagnostics.warn_once(
...
class VNode:
    ...
    def __init__(self, element: Element, children: Optional[List["VNode"]] = None, tag: Optional[int] = None) -> None:
        ...
        # True while an enclosing Suspense boundary hides this native
        # root (``display: "none"`` override) during an update-time
        # suspension.
        self.hidden_by_suspense: bool = False
        # For error-boundary nodes: the caught exception while the
        # fallback is showing, else ``None``.
        self.error: Optional[BaseException] = None
        # For Suspense nodes: whether the fallback is showing; the
        # committed content nodes kept mounted (and hidden) while the
        # fallback shows after an update-time suspension; the hook
