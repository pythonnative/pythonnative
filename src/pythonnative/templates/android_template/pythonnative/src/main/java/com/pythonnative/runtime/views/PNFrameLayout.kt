 ...
 * semantics. Component managers create these for flex containers and
 * drive the mode via [setPointerEventsMode]:
 *
 ...
 *   intercepts or consumes them (the styler also mutes the view's own
 *   touch listeners).
 * - `"box_only"`: the view receives touches and its children don't;
 *   every event in the subtree is intercepted at this level.
 */
open class PNFrameLayout(context: Context) : FrameLayout(context) {
    ...
    /** The current pointer-events mode. */
    fun getPointerEventsMode(): String = pointerEventsMode
